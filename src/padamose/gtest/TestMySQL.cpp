...
//----------------------------------------------------------------//
TEST ( MySQL, hello_mysql ) {
 
    printf ( "The MySQL client version is: %s\n", mysql_get_client_info ());
 
    MYSQL* mysql = mysql_init ( NULL );
    if ( mysql_real_connect ( mysql, "127.0.0.1", "diablouser", "password", "diablo_golf", 3306, NULL, 0 ) == NULL ) {
...
//    uint64_t mysql_insert_id(MYSQL *mysql);
    ...
    mysql_query ( mysql, "SELECT * FROM datadash_users" );
